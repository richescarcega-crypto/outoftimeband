# Phase C6a / r959 — Merge to Main Plan
...
This plan covers merging **only** the bounded C6a / r959 Calendar Important Date Helper stack into production `main`, after Rich’s explicit approval.
...
| HEAD | `6d28907` |
| `origin/modularization-home-layout-engine-pilot` | `6d28907` |
| `origin/main` | `3d2b462` |
...
| Checkpoint commit | `6d28907` |

---

## 2. Exact C6a / r959 Commit Stack

### 2a. Two existing commits (current runtime + checkpoint stack)

| SHA | Message |
|-----|---------|
| `9e6bd75` | Extract Calendar important date helper |
| `6d28907` | Document r959 Calendar important date helper |
...
This is the **current** C6a runtime + checkpoint stack ending at `6d28907`.
...
1. `9e6bd75` — Extract Calendar important date helper  
2. `6d28907` — Document r959 Calendar important date helper  
3. **the merge-plan commit created after this document is committed**

**Fast-forward target:** HEAD after the merge-plan document is committed (not `6d28907` alone, once the merge-plan commit exists).

`origin/main` must still be **`3d2b462`** at merge time.
...
The modularization branch is a **direct descendant** of `origin/main` at `3d2b462`.
...
git merge-base --is-ancestor 3d2b462 $ffTip
# exit code 0 required

git log --oneline 3d2b462..$ffTip
# must show exactly the two existing C6a commits plus the merge-plan commit:
#   9e6bd75, 6d28907, and the post-merge-plan tip
```

Equivalent range: `3d2b462..<post-merge-plan HEAD>`.

**Stop** if `origin/main` is no longer `3d2b462`, or if ancestry / range checks fail.
...
- Do **not** rebase or rewrite the C6a / r959 stack without a new approved plan.
...
- **All inline scripts syntax** check — PASS (expect 8 scripts, 0 failures), same gate used for r959 local validation.
...
After a clean fast-forward of the full C6a / r959 stack (two existing commits plus the merge-plan commit), `main` should introduce or change **only**:

| File | Role |
|------|------|
| `index.html` | Inline `getImportantDatesOn` def removed; r959 Build Version |
| `js/calendar-date-helpers.js` | Important Date helper + namespace keys + legacy alias |
| `tests/integrity/calendar-helpers-package.mjs` | Important Date export / no-inline-def integrity |
| `docs/modularization/PHASE_C6A_R959_CALENDAR_IMPORTANT_DATE_HELPER_CHECKPOINT.md` | Checkpoint doc |
| `docs/modularization/PHASE_C6A_R959_MERGE_TO_MAIN_PLAN.md` | This merge plan (present on tip after the merge-plan commit) |

No other runtime, asset, or test files should appear in the merge diff vs `3d2b462`.
...
C6a is helpers-only: Important Date exact-date matching and recurring **MM-DD** matching remain unchanged; original ordering preserved; no mutation; no sorting, timezone, locale, cleanup, or recurrence-policy change; no render-engine move; public Calendar render ownership (`rCal`, drawers) stays in `index.html`.
...
2. Confirm `origin/main` is still **`3d2b462`**.
3. Confirm post-merge-plan HEAD is a **direct descendant** of `3d2b462` with exactly the two existing C6a commits plus that merge-plan commit.
...
5. **Stop** if:
   - `origin/main` has moved away from `3d2b462`, or
...
3. Confirm Build Version string is `2026-07-15-r959-calendar-important-date-helpers`.
4. Confirm `git log --oneline 3d2b462..HEAD` shows exactly the two existing C6a commits plus the merge-plan commit (`9e6bd75`, `6d28907`, and the post-merge-plan tip).
5. Confirm `git diff --name-only 3d2b462..HEAD` matches the expected file list (Section 6).
...
| `origin/main` | same post-merge-plan tip (was `3d2b462` before the approved FF) |
| Build Version on deployed/served app | `2026-07-15-r959-calendar-important-date-helpers` |
...
After deploy / hard refresh to r959:

- [ ] Confirm Build Version **r959** (`2026-07-15-r959-calendar-important-date-helpers`)
- [ ] Calendar opens
- [ ] Month navigation works
- [ ] Important Dates display
- [ ] Recurring Important Dates display
- [ ] Normal dates remain normal
- [ ] Birthday markers remain correct
- [ ] Holiday markers remain correct
...
If r959 must be reverted after merge/push:

| Ref | Value |
|-----|--------|
| Safe rollback tip | `origin/main` @ **`3d2b462`** (pre-C6a production) |
| Meaning | State before the C6a / r959 stack (two existing commits + merge-plan commit) |

Rollback requires Rich’s explicit approval and a separate, auditable procedure. Prefer restoring `main` to `3d2b462` over ad-hoc file surgery.
...
Bounded fast-forward onto `main` at `3d2b462` of the two existing C6a commits (`9e6bd75` → `6d28907`) **plus** the merge-plan commit created after this document is committed. Fast-forward target is that post-merge-plan HEAD — not `6d28907` alone once the merge-plan commit exists. `origin/main` must still be `3d2b462` until Rich approves. Helpers-only Important Date extraction (exact-date + recurring MM-DD matching, original ordering, no mutation); protected Home/Flyer/Calendar/holiday/birthday/proposal boundaries untouched; rollback tip `3d2b462`.
